...
impl CommandListenerHandler {
    pub fn wait_for_exit(self) {
        self.thread_handle
            .join()
            .expect("Failed to join command listener thread");
    }
            ...
            loop {
                match self.receiver.recv_timeout(Duration::from_millis(1000)) {
                    ...
                    Err(RecvTimeoutError::Timeout) => {
                        ...
                        {
                            self.log_info(
                                "Termination signal received. Exiting command listener thread.",
                            );
                            break;
                        }
                    }
    ...
    fn parse_message(&self, message: &ServerRequest) {
        match message {
            ...
            ServerRequest::TerminateServer => {
                self.log_info(
                    "Received TerminateServer request from ServerController. Processing...",
                );

                match self
                    .command_processor
                    .process(ServerRequest::TerminateServer)
                {
                    Ok(resp) => {
            ...
            ServerRequest::TerminateClient(client_id) => {
                ...
                match self
                    .command_processor
                    .process(ServerRequest::TerminateClient(*client_id))
                {
                    Ok(resp) => {
                        ...
                        self.log_info(&format!(
                            "Received confirmation that client {} terminated.",
                            client_id
                        ));
                        self.sender.send(response).unwrap();
                    }
            ...
            ServerRequest::StopServer => {
                self.log_info("Received StopServer request from ServerController. Processing...");

                match self.command_processor.process(ServerRequest::StopServer) {
                    Ok(resp) => {
                        let stopped = ServerStopped::assert_variant_of(resp);
                        let response = ServerResponse::ServerStopped(stopped);
                        self.log_info("Received confirmation that server stopped.");
                        self.sender.send(response).unwrap();
                    }
                    Err(e) => {
                        let err_msg = format!("Failed to stop server: {}", e);
                        self.log_error(&err_msg);
